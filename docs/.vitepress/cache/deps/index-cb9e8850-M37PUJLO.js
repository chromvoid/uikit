// ../../node_modules/@reatom/core/build/index-cb9e8850.js
...
function createStore(dbName, storeName) {
  ...
  return (txMode, callback) => (() => {
    ...
    return request.onupgradeneeded = () => request.result.createObjectStore(storeName), dbp = promisifyRequest(request), dbp.then((db) => {
...
function setMany(entries2, customStore = defaultGetStore()) {
  return customStore("readwrite", (store) => (entries2.forEach((entry) => store.put(entry[1], entry[0])), promisifyRequest(store.transaction)));
}
...
function update(key, updater, customStore = defaultGetStore()) {
  return customStore("readwrite", (store) => new Promise((resolve, reject) => {
    store.get(key).onsuccess = function() {
      try {
        store.put(updater(this.result), key), resolve(promisifyRequest(store.transaction));
      } catch (err) {
        reject(err);
      }
    };
...
function delMany(keys2, customStore = defaultGetStore()) {
  return customStore("readwrite", (store) => (keys2.forEach((key) => store.delete(key)), promisifyRequest(store.transaction)));
}
...
function entries(customStore = defaultGetStore()) {
  return customStore("readonly", (store) => {
    if (store.getAll && store.getAllKeys) return Promise.all([promisifyRequest(store.getAllKeys()), promisifyRequest(store.getAll())]).then(([keys2, values2]) => keys2.map((key, i) => [key, values2[i]]));
    const items = [];
...
//# sourceMappingURL=index-cb9e8850-M37PUJLO.js.map
